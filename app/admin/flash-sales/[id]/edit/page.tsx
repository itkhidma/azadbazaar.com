...
export default function EditFlashSalePage() {
  ...
  const loadData = async () => {
    try {
      ...
      setFormData({
        ...
        stockLimit: flashSale.stockLimit.toString(),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await updateFlashSale(flashSaleId, updateData);
      router.push('/admin/flash-sales');
    } catch (err: any) {
      setError(err.message || 'Failed to update flash sale');
      setLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-4xl">
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-gray-900">⚡ Edit Flash Sale</h1>
        <p className="text-gray-600 mt-1">Update flash sale details</p>
      </div>
